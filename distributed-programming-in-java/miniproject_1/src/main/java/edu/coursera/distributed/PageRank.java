...
public final class PageRank {
    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        JavaPairRDD<Integer, Double> contributions = sites.join(ranks)
                .flatMapToPair(rankedSite -> {
                    Website srcSite = rankedSite._2()._1();
                    Double srcRank = rankedSite._2()._2();

                    List<Tuple2<Integer, Double>> srcContributions = new ArrayList<>();

                    Iterator<Integer> targetIterator = srcSite.edgeIterator();
                    while (targetIterator.hasNext()) {
                        Integer target = targetIterator.next();
                        Double targetContribution = srcRank / (double)srcSite.getNEdges();
                        srcContributions.add(new Tuple2<>(target, targetContribution));
                    }

                    return srcContributions;
                });

        return contributions.reduceByKey(Double::sum)
                .mapValues(totalContributions -> 0.15 + 0.85 * totalContributions);
    }
}
