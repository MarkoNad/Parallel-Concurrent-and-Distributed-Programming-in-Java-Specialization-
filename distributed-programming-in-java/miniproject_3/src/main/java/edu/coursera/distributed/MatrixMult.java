...
public class MatrixMult {
    ...
    public static void parallelMatrixMultiply(Matrix a, Matrix b, Matrix c,
        ...
        int chunkSize = (nRows + size - 1) / size;
        int startRow = chunkSize * myRank;
        int endRow = chunkSize * (myRank + 1);
        if (endRow > nRows) {
            endRow = nRows;
        }
        ...
        if (myRank == 0) {
            List<MPI.MPI_Request> requests = new ArrayList<>();
            for (int i = 1; i < size; i++) {
                int rankStartRow = chunkSize * i;
                int rankEndRow = chunkSize * (i + 1);
                if (rankEndRow > nRows) {
                    rankEndRow = nRows;
                }
                ...
                MPI.MPI_Request request = mpi.MPI_Irecv(c.getValues(), rowOffset, nElements, i, i, mpi.MPI_COMM_WORLD);
                requests.add(request);
            }
            mpi.MPI_Waitall(requests.toArray(new MPI.MPI_Request[0]));
        } else {
